...
export class User
{
  ...
  static connect_player (g__GameMaps : Map<GameMap_ID, GameMap>, p__GameMap_ID : GameMap_ID, g__Users : Map<string, User>, uuID : string) : { status : Status }
  {
    // @ts-ignore
    const l__GameMap__connect_player__ReVa = GameMap.connect_player(g__GameMaps, p__GameMap_ID, g__Users.get(uuID).player);
  ...
  static connect(g__GameMaps : Map<GameMap_ID, GameMap>, p__GameMap_ID : GameMap_ID, g__Users : Map<string, User>, uuID : string, player_ws__new : WebSocket) : { status : Status, wasUserAlreadyConnected : boolean, player_ws__old : (WebSocket|undefined) }
  {
    const ssID = v4.generate();
    
    
    // @ts-ignore
    const wasUserAlreadyConnected = ((g__Users.get(uuID) == undefined) ? false : g__Users.get(uuID).#isConnected);
    ...
    const ReVa__User__connect_player = User.connect_player(g__GameMaps, p__GameMap_ID, g__Users, uuID);
    
    return ({ status: ReVa__User__connect_player.status, wasUserAlreadyConnected, player_ws__old });
  };
  
  static disconnect_player(g__GameMaps : Map<GameMap_ID, GameMap>, g__Users : Map<string, User>, uuID : string) : { status : Status }
  {
    const l__GameMap__disconnect_player__ReVa = GameMap.disconnect_player(g__GameMaps, uuID);
  ...
  static disconnect(g__GameMaps : Map<GameMap_ID, GameMap>, g__Users : Map<string, User>, uuID : string)
  {
    ...
    // @ts-ignore
    if (g__Users.get(uuID).#isConnected)
    {
      const ReVa__User__disconnect_player = User.disconnect_player(g__GameMaps, g__Users, uuID);
